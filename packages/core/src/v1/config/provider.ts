    ...
    Schema.StructWithRest(
      Schema.Struct({
        ...
        rateLimit: Schema.optional(
          Schema.Struct({
            requestsPerMinute: Schema.optional(PositiveInt).annotate({
              description: "Maximum API requests per minute (token bucket).",
            }),
            tokensPerMinute: Schema.optional(PositiveInt).annotate({
              description: "Maximum tokens per minute (reserved for future use).",
            }),
          }).annotate({ description: "Rate limiting configuration for this provider." }),
        ).annotate({ description: "Rate limiting configuration." }),
      }),
      [Schema.Record(Schema.String, Schema.Any)],
    ),
  ),
