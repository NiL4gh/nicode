...
import {
  ...
  isConfigCommand,
  movePromptHistory,
  parseRateLimitCommand,
  parseConfigCommand,
  pushPromptHistory,
} from "./prompt.shared"
import { OPENCODE_BASE_MODE, useBindings } from "@opencode-ai/tui/keymap"
import { realignEditorPromptParts, resolveEditorSlashValue } from "./prompt.editor"
import { setRateLimit, getRateLimit, getAllRateLimits } from "@/provider/rate-limit"
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  function handleLocalCommand(text: string, inp: PromptInput): boolean {
    const t = text.trim()
    const lc = t.toLowerCase()

    // /help
    if (isHelpCommand(text)) {
      inp.onStatus("/config /settings — show/change settings | /cost — show usage | /clear — clear screen | /doctor — check setup | /rate-limit — adjust rate limits | /exit /quit — exit")
      return true
    }
    ...
    if (isCostCommand(text)) {
      const st = inp.state()
      const parts = [`Model: ${st.model}`]
      if (st.usage) parts.push(`Usage: ${st.usage}`)
      if (st.duration) parts.push(`Duration: ${st.duration}`)
      inp.onStatus(parts.join(" | "))
      return true
    }

    // /doctor
    if (isDoctorCommand(text)) {
      const st = inp.state()
      const lines: string[] = [`Model: ${st.model}`, `Phase: ${st.phase}`]
      if (st.usage) lines.push(`Usage: ${st.usage}`)
      inp.onStatus(lines.join(" | "))
      return true
    }

    // /config or /settings
    if (isConfigCommand(text)) {
      const action = parseConfigCommand(text)
      if (!action) {
        inp.onStatus("Usage: /config [show] | /config set <key> <value> | Keys: diff_style (stacked|auto)")
        return true
      }
      if (action.action === "set") {
        const { key, value } = action
        if (key === "diff_style" && (value === "stacked" || value === "auto")) {
          inp.onStatus(`Set diff_style to ${value} (persist by editing opencode.jsonc → tui.diff_style)`)
        } else {
          inp.onStatus(`Unknown setting: ${key}. To persist, edit opencode.jsonc directly.`)
        }
      } else {
        const st = inp.state()
        const parts = [`Model: ${st.model}`, `Diff: ${inp.tuiConfig.diff_style}`, `Queue: ${st.queue}`]
        if (st.usage) parts.push(`Usage: ${st.usage}`)
        inp.onStatus(parts.join(" | "))
      }
      return true
    ...
    // /rate-limit
    if (isRateLimitCommand(text)) {
      const action = parseRateLimitCommand(text)
      if (!action) {
        inp.onStatus("Usage: /rate-limit [show] | /rate-limit set <provider> <rpm>")
        return true
      }
      if (action.action === "set") {
        setRateLimit(action.provider, action.rpm)
        inp.onStatus(`Rate limit set: ${action.provider} → ${action.rpm} RPM`)
      } else {
        const all = getAllRateLimits()
        const entries = Object.entries(all)
        if (entries.length === 0) {
          inp.onStatus("No rate limits configured (all providers are unlimited)")
        } else {
          const lines = entries.map(([k, v]) =>
            v.unlimited ? `${k}: unlimited` : `${k}: ${v.rpm} RPM (${v.available} tokens available)`
          )
          inp.onStatus(lines.join(" | "))
        }
      }
  ...
  const submitPrompt = (next: RunPrompt) => {
    ...
    if (!command && next.mode !== "shell") {
      resetDraft()
      if (handleLocalCommand(next.text, input)) return
    }
