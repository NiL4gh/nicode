  ...
  Effect.gen(function* () {
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        const rateLimit = options["rateLimit"] as { requestsPerMinute?: number; tokensPerMinute?: number } | undefined
        delete options["chunkTimeout"]
        delete options["headerTimeout"]
        delete options["rateLimit"]

        const rpmLimit = rateLimit?.requestsPerMinute ?? 1_000_000

        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          await acquireToken(model.providerID, rpmLimit)
