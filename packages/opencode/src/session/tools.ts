...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  const allMcp = yield* mcp.tools()
  const servers = new Map<string, Array<{ key: string; entry: MCP.McpTool }>>()
  for (const [key, entry] of Object.entries(allMcp)) {
    const suffix = "_" + McpCatalog.sanitize(entry.def.name)
    const serverName = key.endsWith(suffix) ? key.slice(0, key.length - suffix.length) : key
    if (!servers.has(serverName)) servers.set(serverName, [])
    servers.get(serverName)!.push({ key, entry })
  }

  for (const [serverName, serverTools] of servers) {
    const proxyName = `mcp_${serverName}`
    const toolList = serverTools.map(
      (t) => `- ${t.entry.def.name}: ${(t.entry.def.description ?? "").slice(0, 120)}`,
    )
    const proxyDescription =
      `Execute a tool on the "${serverName}" MCP server. Available tools:\n${toolList.join("\n")}\n` +
      `Use tools_search first to get the full parameter schema for any tool before calling it.`
    ...
    const proxySchema = {
      ...
      properties: {
        tool: {
          ...
          enum: serverTools.map((t) => t.entry.def.name),
          description: `The tool name from the "${serverName}" server`,
        },
        arguments: {
          type: "object" as const,
          description: "Arguments to pass to the tool. Use tools_search to discover the parameter schema.",
    ...
    tools[proxyName] = tool({
      description: proxyDescription,
      inputSchema: jsonSchema(proxySchema),
      execute(args, opts) {
        return run.promise(
          Effect.gen(function* () {
            const toolInput = toRecord(args)
            const toolName = typeof toolInput.tool === "string" ? toolInput.tool : ""
            const toolArgs =
              typeof toolInput.arguments === "object" && toolInput.arguments !== null
                ? (toolInput.arguments as Record<string, unknown>)
                : {}

            const toolKey = `${serverName}_${McpCatalog.sanitize(toolName)}`
            const mcpEntry = allMcp[toolKey]
            if (!mcpEntry) {
              const avail = serverTools.map((t) => t.entry.def.name).join(", ")
              throw new Error(`Tool "${toolName}" not found on "${serverName}". Available: ${avail}`)
            }

            const item = McpCatalog.convertTool(mcpEntry.def, mcpEntry.client, mcpEntry.timeout)
            const execute = item.execute
            if (!execute) throw new Error(`Tool "${toolName}" has no execute handler`)

            const ctx = context(toolArgs, opts)
            yield* plugin.trigger(
              "tool.execute.before",
              { tool: proxyName, sessionID: ctx.sessionID, callID: opts.toolCallId },
              { args: { tool: toolName, arguments: toolArgs } },
            )
            yield* ctx.ask({ permission: `mcp:${serverName}:${toolName}`, metadata: {}, patterns: ["*"], always: ["*"] })
            ...
            const output = {
              ...
              attachments: attachments.map((attachment) => ({
                ...
                id: PartID.ascending(),
                sessionID: ctx.sessionID,
                messageID: resolveInput.processor.message.id,
              })),
              content: result.content,
            }
            if (opts.abortSignal?.aborted) {
              yield* resolveInput.processor.completeToolCall(opts.toolCallId, output)
            }
            return output
          }),
        )
